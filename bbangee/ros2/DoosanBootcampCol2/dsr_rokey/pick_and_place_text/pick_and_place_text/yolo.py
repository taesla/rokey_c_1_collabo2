...
YOLO_MODEL_FILENAME = "best_tools.pt"
...
class YoloModel:
    ...
    def get_best_detection_with_viz(self, img_node, target):
        ...
        for det in detections:
            ...
            score = det["score"]
            label = det["label"]
            class_name = list(self.reversed_class_dict.keys())[list(self.reversed_class_dict.values()).index(label)]
            
            x1, y1, x2, y2 = map(int, box)
            # 타겟 클래스는 녹색, 나머지는 파란색
            color = (0, 255, 0) if label == label_id else (255, 0, 0)
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(annotated_frame, f"{class_name} {score:.2f}", 
        ...
        best_det = max(matches, key=lambda x: x["score"])
        return best_det["box"], best_det["score"], annotated_frame
