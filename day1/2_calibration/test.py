...
class TestNode(Node):
    ...
    def pick_and_drop(self, x, y, z):
        ...
        # Move to above target (100mm above for safety)
        pick_pos_above = posx([x, y, z + 10, current_pos[3], current_pos[4], current_pos[5]])
        print(f"[Test] Moving above target: X={x:.1f}, Y={y:.1f}, Z={z+100:.1f}")
        movel(pick_pos_above, vel=VELOCITY, acc=ACC)
        wait(0.5)
        
        # Move down to target
        pick_pos = posx([x, y, z, current_pos[3], current_pos[4], current_pos[5]])
        print(f"[Test] Moving to target: Z={z:.1f}")
        movel(pick_pos, vel=VELOCITY/2, acc=ACC/2)
        wait(0.5)
